...
def createFastaFile(sequences):
    with open('tmp/potentialofftargetstrings.fa', 'w') as f:
        for i in sequences:
...
# Only called for offline mode.
def dbToCsv():
    with open(os.path.join(base_path, 'offtargets.csv'), 'w') as nf:
...
def filterPotentialOffTargets(potentialOffTargetStrings, refGenome, H, rule, filter, mismatches, whereabouts):
    print('Locating off-targets...\n')
    createFastaFile(potentialOffTargetStrings)
    bowtieSearch('{0} {1} -f tmp/potentialofftargetstrings.fa -k 200 -v 3 tmp/output.out'.format(bowtiePath, refGenome))
    ht = {0:0, 1:0, 2:0, 3:0}
    prevLine = 1
    potentialOffTargets = {}
    summary = {}
...
def main(args):
    ...
    offTargSummary, potentialOffTargets = filterPotentialOffTargets(
        potentialOffTargetStrings, os.path.join(refGenomeDir,args.refGenome), args.mismatches, args.rule, filterRegex, args.mismatches, whereabouts)
    insertIntoDB(
